...
export async function saveCitizenToFirestore(citizen: CitizenAccount): Promise<void> {
  try {
    ...
    await setDoc(doc(db, USERS_COLLECTION, docId), {
      id: docId,
      citizen_id: citizen.citizen_id || docId,
      username: citizen.username,
      full_name: citizen.full_name,
      email: (citizen.email || '').toLowerCase().trim(),
...
/**
 * Fetch a specific citizen profile from Firestore by User UID / doc ID
 */
export async function getCitizenProfileFromFirestore(userId: string): Promise<CitizenAccount | null> {
  try {
    ...
    console.warn('[Auth] Direct profile fetch note:', e);
  }
  return null;
}
...
export async function syncUsersFromFirestore(): Promise<CitizenAccount[]> {
  try {
    ...
    // Offline fallback or non-admin permission boundary
  }
...
export async function registerCitizenAsync(data: {
  ...
  // 5. Account Consistency Check - Prevent Duplicate Email or Phone in local registry
  const citizens = getRegisteredCitizens();
...
export async function loginCitizenAsync(
  ...
  if (targetEmail) {
    try {
      ...
      // Fetch permanent profile from Firestore using UID
      const remoteProfile = await getCitizenProfileFromFirestore(firebaseUser.uid);

      const resolvedCitizenId = remoteProfile?.citizen_id || found?.citizen_id || `CIT-${firebaseUser.uid.slice(0, 5).toUpperCase()}`;
      const resolvedFullName = remoteProfile?.full_name || found?.full_name || firebaseUser.displayName || 'Citizen';
      const resolvedUsername = remoteProfile?.username || found?.username || firebaseUser.displayName?.split(' ')[0] || 'Citizen';
      const resolvedPhone = sanitizePhone(remoteProfile?.phone || found?.phone || firebaseUser.phoneNumber || '');
      ...
      const userProfile: UserProfile = {
        ...
        profile_photo: remoteProfile?.profile_photo || found?.profile_photo || firebaseUser.photoURL || undefined,
        emailVerified: firebaseUser.emailVerified,
        phoneVerified: remoteProfile?.phoneVerified || found?.phoneVerified || Boolean(firebaseUser.phoneNumber),
        created_at: remoteProfile?.created_at || found?.created_at || new Date().toISOString()
      };

      // Save to local registry if missing
      const existingInList = citizens.findIndex(c => c.id === firebaseUser.uid || c.citizen_id === resolvedCitizenId || c.email === userProfile.email);
      if (existingInList === -1) {
        citizens.push({
          id: firebaseUser.uid,
          citizen_id: resolvedCitizenId,
      ...
      // If document was not in Firestore, save it now
      if (!remoteProfile) {
        await saveCitizenToFirestore({
          id: firebaseUser.uid,
          citizen_id: resolvedCitizenId,
          username: resolvedUsername,
          full_name: resolvedFullName,
...
export function verifyOfficerCredentials(
  ...
  if (!cleanId) {
    return { success: false, error: 'Please provide your Officer Email or Officer ID.' };
  }
  if (!cleanPass) {
    return { success: false, error: 'Please enter your Officer Password / PIN.' };
  }

  const found = officersList.find(o => {
    const matchId = (o.id || '').toLowerCase() === cleanId || (o.officer_id || '').toLowerCase() === cleanId;
    const matchEmail = (o.email || '').toLowerCase() === cleanId || 
                       `${o.name.toLowerCase().replace(/\s+/g, '.')}@civicmind.gov.in` === cleanId || 
                       `${o.name.toLowerCase().replace(/\s+/g, '.')}@municipal.gov.in` === cleanId;
    const matchUsername = (o.username || '').toLowerCase() === cleanId;
    const matchName = o.name.toLowerCase() === cleanId;
    return matchId || matchEmail || matchUsername || matchName;
  });

  if (!found) {
    return { 
      success: false, 
      error: 'Invalid officer credentials. Officer account not found in municipal registry.' 
    };
  }
  ...
  if (found.is_active === false) {
    return {
      success: false,
      error: 'This officer account is inactive. Please contact Municipal Administration.'
    };
  }
  ...
  const isPassValid = validPins.some(p => p.toLowerCase() === cleanPass.toLowerCase());

  if (!isPassValid) {
    return {
      success: false,
      error: 'Invalid officer credentials. Incorrect password.'
    };
  }
...
export function canAccessGovernmentPortal(user: UserProfile | null): boolean {
  ...
  // Normal citizens are strictly forbidden from accessing the Government Portal
  if (user.role === 'CITIZEN' && !isAuthorizedAdminOrOwner(user)) {
    return false;
  }

  return isAuthorizedAdminOrOwner(user) || user.role === 'GOVERNMENT_ADMIN';
}

export function canAccessOfficerPortal(user: UserProfile | null, officer?: DepartmentOfficer | null): boolean {
  if (user && isAuthorizedAdminOrOwner(user)) {
    return true;
  }

  // Normal citizens cannot access the Officer Portal
  if (user?.role === 'CITIZEN' && !officer) {
    return false;
  }
...
export function isAuthorizedAdminOrOwner(user: UserProfile | null): boolean {
  if (!user) return false;
  ...
  if (
    email === 'vu.241fa04491@gmail.com' ||
    email === 'owner@civicmind.gov.in' ||
    (phone.length >= 10 && phone.includes('8121312412'))
  ) {
    return true;
  }

  if ((user.role === 'OWNER' || user.role === 'SYSTEM_ADMIN') && (email.includes('241fa04491') || email.includes('owner'))) {
    return true;
  }
...
export function isOwnerUser(user: UserProfile | null): boolean {
  return isAuthorizedAdminOrOwner(user);
}
