...
export function runClassificationAgent(
  ...
  let resolvedCat: CivicCategory = 'Road Damage';
  let subcategory = 'Pothole & Cavitation';
  let problemType = 'Road Surface Degradation';
  const urgency: string[] = [];

  if (text.includes('pothole') || text.includes('asphalt') || text.includes('footpath') || text.includes('pavement') || text.includes('road') || text.includes('crater')) {
  ...
  } else if (text.includes('water supply') || text.includes('potable') || text.includes('drinking water') || text.includes('water pipe') || text.includes('pipeline') || (text.includes('water') && !text.includes('drain') && !text.includes('sewage'))) {
    resolvedCat = 'Water Supply';
    subcategory = 'Pipeline Leakage / Pressure Deficit';
    problemType = 'Potable Water Distribution Disruption';
  } else if (text.includes('garbage') || text.includes('waste') || text.includes('dump') || text.includes('trash') || text.includes('litter') || text.includes('sanitation')) {
    resolvedCat = 'Garbage / Sanitation';
    subcategory = 'Solid Waste Overflow';
    problemType = 'Public Sanitation Accumulation';
  } else if (text.includes('drain') || text.includes('sewage') || text.includes('gutter') || text.includes('culvert') || text.includes('overflow') || text.includes('manhole')) {
    resolvedCat = 'Drainage';
    subcategory = 'Stormwater / Sewage Overflow';
    problemType = 'Sanitary Drainage Blockage';
  } else if (text.includes('street light') || text.includes('streetlight') || text.includes('dark street') || text.includes('lamp') || text.includes('illumination')) {
    resolvedCat = 'Streetlights';
    subcategory = 'Streetlight Outage / Dark Corridor';
    problemType = 'Public Illumination Failure';
  } else if (text.includes('mosquito') || text.includes('dengue') || text.includes('fumigation') || text.includes('hygiene') || text.includes('health') || text.includes('epidemic')) {
    resolvedCat = 'Health / Sanitation Hazard';
    subcategory = 'Vector Breeding / Health Hazard';
    problemType = 'Public Health & Disease Vector Risk';
  } else if (text.includes('park') || text.includes('garden') || text.includes('playground') || text.includes('swing') || text.includes('slide') || text.includes('bench') || text.includes('tree')) {
    resolvedCat = 'Parks & Greenery';
    subcategory = 'Park Amenities & Vegetation';
    problemType = 'Public Recreation Area Upkeep';
  } else if (text.includes('traffic') || text.includes('signal') || text.includes('zebra') || text.includes('speed breaker') || text.includes('signage')) {
    resolvedCat = 'Traffic & Signals';
    subcategory = 'Traffic Control & Signage';
    problemType = 'Traffic Flow & Safety Obstruction';
  } else if (text.includes('transformer') || text.includes('spark') || text.includes('power cut') || text.includes('electric wire') || text.includes('cable') || text.includes('electricity')) {
    resolvedCat = 'Electricity';
    subcategory = 'High Voltage / Transformer Hazard';
    problemType = 'Electrical Power Grid Hazard';
  } else if (text.includes('public toilet') || text.includes('bus shelter') || text.includes('building') || text.includes('facility') || text.includes('property')) {
    resolvedCat = 'Public Buildings';
    subcategory = 'Municipal Facility Maintenance';
    problemType = 'Public Asset Deterioration';
  } else {
    resolvedCat = 'Road Damage';
    subcategory = 'General Civic Infrastructure';
    problemType = 'Civic Maintenance Request';
  }
...
export function runDepartmentRoutingAgent(
  ...
  let deptKey: CivicDepartmentKey = 'roads';
  let deptName = 'Roads & Infrastructure';

  if (text.includes('pothole') || text.includes('asphalt') || text.includes('footpath') || text.includes('pavement') || text.includes('road') || text.includes('crater') || text.includes('bridge') || text.includes('median')) {
    deptKey = 'roads';
    ...
    deptKey = 'drainage';
    deptName = 'Drainage & Sewage';
  } else if (text.includes('street light') || text.includes('streetlight') || text.includes('dark street') || text.includes('lamp') || text.includes('illumination')) {
    deptKey = 'street_lighting';
    deptName = 'Street Lighting';
  } else if (text.includes('mosquito') || text.includes('dengue') || text.includes('fumigation') || text.includes('hygiene') || text.includes('health') || text.includes('epidemic')) {
    deptKey = 'public_health';
    deptName = 'Public Health';
  } else if (text.includes('park') || text.includes('garden') || text.includes('playground') || text.includes('swing') || text.includes('slide') || text.includes('bench') || text.includes('tree')) {
    deptKey = 'parks';
    deptName = 'Parks & Public Spaces';
  } else if (text.includes('traffic') || text.includes('signal') || text.includes('zebra') || text.includes('speed breaker') || text.includes('signage')) {
    deptKey = 'traffic';
    deptName = 'Traffic & Transportation';
  } else if (text.includes('transformer') || text.includes('spark') || text.includes('power cut') || text.includes('electric wire') || text.includes('cable') || text.includes('electricity')) {
    deptKey = 'electricity';
    deptName = 'Electricity & Utilities';
  } else if (text.includes('public toilet') || text.includes('bus shelter') || text.includes('building') || text.includes('facility') || text.includes('property')) {
    deptKey = 'public_facilities';
    deptName = 'Public Facilities & Buildings';
  } else {
    deptKey = 'roads';
    deptName = 'Roads & Infrastructure';
  }
