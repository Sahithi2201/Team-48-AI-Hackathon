...
export const CitizenDashboardPage: React.FC<CitizenDashboardPageProps> = ({
  ...
  const userCitizenId = (currentUser?.citizen_id || '').trim().toLowerCase();
  ...
  const citizenCases = cases.filter((c) => {
    ...
    // Check match by Firebase User UID or internal User ID
    const cUserId = (c.userId || '').trim().toLowerCase();
    if (cUserId && (cUserId === userId || cUserId === userCitizenId)) {
      return true;
    }

    // Check match by Citizen ID
    const cCitizenId = (c.citizenId || '').trim().toLowerCase();
    if (cCitizenId && (cCitizenId === userCitizenId || cCitizenId === userId)) {
      return true;
    }
