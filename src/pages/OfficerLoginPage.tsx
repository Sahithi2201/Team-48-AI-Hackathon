...
export const OfficerLoginPage: React.FC<OfficerLoginPageProps> = ({
  ...
  const handleVerifyAndLogin = (e?: React.FormEvent) => {
    ...
    const cleanEmail = officerEmail.trim();
    const cleanPass = officerPassword.trim();

    if (!cleanEmail) {
      setErrorMsg('Please enter your authorized Officer Email address or Badge ID.');
      return;
    }
    ...
    const validation = validateOfficerCredentials(cleanEmail, cleanPass);
    if (!validation.isValid || !validation.officer) {
      setIsVerifying(false);
      setErrorMsg(validation.message || 'Invalid officer credentials. Access denied.');
      return;
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
          ...
          <div className="lg:col-span-7 p-6 sm:p-7 border-b lg:border-b-0 lg:border-r border-slate-200 flex flex-col justify-between space-y-5 bg-slate-50/50">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-slate-500 font-medium mt-0.5">
                            {officer.email || `${officer.name.toLowerCase().replace(/\s+/g, '.')}@civicmind.gov.in`}
                          </p>
                          <p className="text-[10px] text-slate-400 font-medium">
                            {officer.departmentName} • {officer.area || officer.city}
                          </p>
                        </div>
                      </div>
            ...
            <div className="p-3 bg-blue-50/80 rounded-xl border border-blue-200 text-xs text-blue-900 flex items-start gap-2 mt-2">
              ...
              <span>
                <strong>Authorized Officers:</strong> 50 pre-registered municipal officers across all 5 key departments. Select any officer or enter their email + password to verify access.
              </span>
            </div>
            ...
            <form onSubmit={handleVerifyAndLogin} className="space-y-4">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Authenticate your official credentials to access your isolated officer task workspace.
                </p>
              </div>
              ...
              {selectedOfficer && (
                <div className="p-4 rounded-2xl bg-gradient-to-br from-slate-900 to-blue-950 text-white space-y-3 shadow-md">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-cyan-300 mt-0.5">{selectedOfficer.designation || 'Field Lead Officer'} • {selectedOfficer.area}, {selectedOfficer.city}</p>
                  </div>
              ...
              {/* 1. Officer Email Input */}
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 flex items-center gap-1.5">
                  <UserCheck className="w-3.5 h-3.5 text-slate-400" />
                  <span>Officer Official Email / ID</span>
                </label>
                <input
                  type="text"
                  value={officerEmail}
                  ...
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-900 focus:outline-hidden focus:ring-2 focus:ring-blue-600 focus:bg-white"
                  placeholder="e.g. ravi.kumar@civicmind.gov.in or OFF-001"
                />
              </div>

              {/* 2. Officer Password Input */}
              <div className="space-y-1.5">
