...
export const ReportIssuePage: React.FC<ReportIssuePageProps> = ({
  ...
  const handleSubmitComplaint = async () => {
    ...
    try {
      ...
      // Strict 10 Supported Civic Locations Validation
      const civicServiceCheck = validateCivicServiceLocation({
        cityName: cityName.trim(),
        areaName: areaName.trim(),
        colonyName: colonyName.trim(),
        formattedAddress: selectedFormattedAddress || streetAddress.trim(),
        lat: latitude,
        lng: longitude
      });

      if (!civicServiceCheck.isSupported) {
        setCurrentStep(2);
        throw new Error(civicServiceCheck.errorMessage || `CivicMind is currently operational only in: ${SUPPORTED_LOCATIONS_SUMMARY}. Please choose a supported service area.`);
      }
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1">
                ...
                Supported Municipal Locations (10 Cities Only)
              </span>
            ...
            <div className="flex flex-wrap gap-1.5 pb-1">
              {SUPPORTED_CIVIC_LOCATIONS.map((loc) => {
                const isSelected = cityName.trim().toLowerCase() === loc.name.toLowerCase();
                return (
                  <button
                    key={loc.id}
                    type="button"
                    onClick={() => {
                      ...
                      if (!areaName || areaName === 'Mamillagudem' || areaName === 'Benz Circle') {
                        setAreaName(loc.popularAreas[0] || 'Central Zone');
                      }
                      if (!colonyName || colonyName.includes('Locality')) {
                        setColonyName(`${loc.popularAreas[0] || loc.name} Sector`);
                      }
                      setLatitude(loc.center.lat);
                      setLongitude(loc.center.lng);
                      setSelectedFormattedAddress(`${loc.popularAreas[0] || 'Central Area'}, ${loc.name}, ${loc.district}, ${loc.state}, India`);
                      setIsLocationConfirmed(true);
                    }}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                required
              />
              <datalist id="supported-cities-list">
                {SUPPORTED_CIVIC_LOCATIONS.map(c => (
                  <option key={c.id} value={c.name}>{c.name} ({c.jurisdictionZone})</option>
                ))}
              </datalist>
            </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-slate-100">
            ...
            <button
              onClick={() => {
                ...
                // Check 10 supported cities
                const check = validateCivicServiceLocation({
                  cityName: cityName.trim(),
                  areaName: areaName.trim(),
                  colonyName: colonyName.trim(),
                  formattedAddress: selectedFormattedAddress || streetAddress.trim(),
                ...
                if (!check.isSupported) {
                  setErrorMessage(check.errorMessage || `CivicMind is currently operational only in: ${SUPPORTED_LOCATIONS_SUMMARY}. Please choose a supported service area.`);
                  return;
                }
