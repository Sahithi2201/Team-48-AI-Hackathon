...
export const GoogleMapLocationPickerModal: React.FC<GoogleMapLocationPickerModalProps> = ({
  ...
  // Default coordinates (Vijayawada / Khammam regional default)
  const [markerPosition, setMarkerPosition] = useState<{ lat: number; lng: number }>({
    lat: initialLat && initialLat !== 0 ? initialLat : 16.5062,
    lng: initialLng && initialLng !== 0 ? initialLng : 80.6480,
  });

  const [currentAddress, setCurrentAddress] = useState<string>(
    initialAddress || 'Benz Circle, Vijayawada, NTR District, Andhra Pradesh, India'
  );

  const [parsedData, setParsedData] = useState<Partial<SelectedLocationData>>({
    lat: initialLat && initialLat !== 0 ? initialLat : 16.5062,
    lng: initialLng && initialLng !== 0 ? initialLng : 80.6480,
    formattedAddress: initialAddress || 'Benz Circle, Vijayawada, NTR District, Andhra Pradesh, India',
    city: 'Vijayawada',
    area: 'Benz Circle',
    colony: 'Patamata Locality',
    street: 'MG Road',
    postalCode: '520010',
  });
  ...
  // Real-time Service Area Validation (Strict 10-City Enforcement)
  const serviceValidation = useMemo(() => {
    return validateCivicServiceLocation({
      cityName: parsedData.city,
      areaName: parsedData.area,
      colonyName: parsedData.colony,
      formattedAddress: currentAddress,
      postalCode: parsedData.postalCode,
      lat: markerPosition.lat,
      lng: markerPosition.lng
    });
  }, [parsedData, currentAddress, markerPosition]);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (initialAddress) {
        ...
        reverseGeocodeCoords(16.5062, 80.6480).then(res => {
  ...
  // Jump to specific supported city
  const handleJumpToSupportedCity = async (loc: SupportedCivicLocation) => {
    const newPos = { lat: loc.center.lat, lng: loc.center.lng };
    setMarkerPosition(newPos);
    const geocoded = await reverseGeocodeCoords(loc.center.lat, loc.center.lng);
    const resolvedCity = loc.name;
    const resolvedArea = loc.popularAreas[0] || 'Central Zone';
    const cleanAddr = `${resolvedArea}, ${resolvedCity}, ${loc.district}, ${loc.state}, India`;
    ...
    setParsedData({
      ...
      city: resolvedCity,
      area: resolvedArea,
      colony: `${resolvedArea} Sector`,
      state: loc.state,
      country: 'India',
      postalCode: loc.postalCodes[0] || '520001',
      formattedAddress: cleanAddr,
      lat: newPos.lat,
      lng: newPos.lng
    });
    setLocationStatus(`Jumped to ${loc.name} (${loc.jurisdictionZone})`);
  };
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          ...
          const check = validateCivicServiceLocation({ lat, lng, cityName: geocoded.city, formattedAddress: geocoded.formattedAddress });
          if (check.isSupported) {
            setLocationStatus(`GPS matched ${check.matchedLocation?.name} service zone.`);
          } else {
            setLocationStatus(`GPS detected outside service area. Please select one of 10 supported cities.`);
          }
        },
        async (err) => {
          console.warn('Geolocation failed:', err);
          setIsLocating(false);
          // Fallback to default supported location
          const fallbackPos = { lat: 16.5062, lng: 80.6480 };
          setMarkerPosition(fallbackPos);
          const geocoded = await reverseGeocodeCoords(fallbackPos.lat, fallbackPos.lng);
          setCurrentAddress(geocoded.formattedAddress);
          setParsedData(geocoded);
          setLocationStatus('Could not access GPS. Please click or drag pin within supported cities.');
        },
  ...
  // CONFIRM LOCATION
  const handleConfirm = () => {
    if (!serviceValidation.isSupported) {
      alert(`Cannot select location outside supported service areas.\n\nCivicMind operates strictly in:\n${SUPPORTED_LOCATIONS_SUMMARY}.\n\nPlease choose a location within these 10 cities.`);
      return;
    }

    const matched = serviceValidation.matchedLocation;
    const finalCity = matched?.name || parsedData.city || 'Vijayawada';

    const finalData: SelectedLocationData = {
      lat: parsedData.lat || markerPosition.lat,
      lng: parsedData.lng || markerPosition.lng,
      formattedAddress: currentAddress || parsedData.formattedAddress || `${finalCity}, India`,
      city: finalCity,
      area: parsedData.area || matched?.popularAreas[0] || 'Central Zone',
      colony: parsedData.colony || parsedData.area || `${finalCity} Locality`,
      street: parsedData.street || '',
      state: matched?.state || parsedData.state || 'Andhra Pradesh',
      country: parsedData.country || 'India',
      postalCode: parsedData.postalCode || matched?.postalCodes[0] || '520001',
      landmark: parsedData.landmark || matched?.jurisdictionZone || '',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      <div className="bg-white w-full max-w-3xl rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col h-[92vh] max-h-[720px]">
        ...
        <div className="px-4 sm:px-5 py-3.5 bg-slate-900 text-white flex items-center justify-between border-b border-slate-800 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] sm:text-[11px] text-slate-400">Restricted strictly to 10 authorized municipal service jurisdictions</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-3 bg-slate-50 border-b border-slate-200 flex flex-col gap-2.5 shrink-0">
            ...
            <div className="relative flex-1 w-full">
              <form onSubmit={handleSearchSubmit} className="relative flex items-center">
                ...
                <input
                  ...
                  placeholder="Search in supported cities (e.g. Vijayawada, Guntur, Khammam, Tenali, Bapatla)..."
          ...
          {/* Quick Supported Cities Chips Bar */}
          <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 scrollbar-none text-[11px]">
            <span className="text-[10px] font-extrabold uppercase text-slate-500 tracking-wider shrink-0 flex items-center gap-1 mr-1">
              <Building2 className="w-3 h-3 text-blue-600" />
              10 Service Areas:
            </span>
            {SUPPORTED_CIVIC_LOCATIONS.map((loc) => {
              const isSelected = serviceValidation.isSupported && serviceValidation.matchedLocation?.id === loc.id;
              return (
                <button
                  key={loc.id}
                  type="button"
                  onClick={() => handleJumpToSupportedCity(loc)}
                  className={`px-2.5 py-1 rounded-lg font-bold whitespace-nowrap text-[11px] transition-all cursor-pointer border ${
                    isSelected
                      ? 'bg-blue-600 text-white border-blue-700 shadow-xs'
                      : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-100 hover:text-blue-700'
                  }`}
                  title={`${loc.name} (${loc.jurisdictionZone})`}
                >
                  {loc.name}
                </button>
              );
        ...
        <div className="relative flex-1 w-full min-h-0 bg-slate-100 overflow-hidden">
          ...
          <div className="absolute top-3 left-3 pointer-events-none z-10 hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-900/80 backdrop-blur-md text-white text-[11px] font-medium shadow-md">
            ...
            <span>Click map or drag the pin inside supported jurisdictions</span>
          </div>

          {/* Service Area Status Badge */}
          <div className="absolute bottom-3 left-3 pointer-events-none z-10">
            {serviceValidation.isSupported ? (
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-emerald-950/90 border border-emerald-500/40 text-emerald-200 text-[11px] font-bold shadow-lg backdrop-blur-md">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                <span>Active Service Area: {serviceValidation.matchedLocation?.name} ({serviceValidation.matchedLocation?.district})</span>
              </div>
            ) : (
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-rose-950/90 border border-rose-500/50 text-rose-200 text-[11px] font-bold shadow-lg backdrop-blur-md animate-bounce">
                <ShieldAlert className="w-3.5 h-3.5 text-rose-400" />
                <span>Outside Service Boundary</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-3.5 sm:p-4 bg-white border-t border-slate-200 shrink-0 space-y-3 z-20 shadow-[0_-4px_16px_rgba(0,0,0,0.06)]">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-extrabold border border-emerald-300">
                    ✓ SUPPORTED AREA: {serviceValidation.matchedLocation?.name.toUpperCase()}
                  </span>
                </div>
                ...
                <span className="text-[11px] text-emerald-700 font-bold block mt-1">
                  ✓ {serviceValidation.matchedLocation?.jurisdictionZone || 'Municipal Ward Verified'} • {locationStatus || 'Ready'}
                </span>
              </div>
            </div>
          ) : (
            <div className="flex items-start gap-2.5 bg-rose-50 p-3 sm:p-3.5 rounded-xl sm:rounded-2xl border border-rose-300 animate-in fade-in">
              <ShieldAlert className="w-5 h-5 text-rose-600 shrink-0 mt-0.5" />
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[10px] font-black text-rose-900 uppercase tracking-wider">
                    OUTSIDE CIVIC SERVICE BOUNDARY
                  </span>
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-rose-200 text-rose-900 font-extrabold">
                    CANNOT SELECT
                  </span>
                </div>
                <p className="text-xs font-bold text-rose-800 mt-1 leading-snug">
                  {currentAddress || 'Selected location'} is outside CivicMind municipal service areas.
                </p>
                <p className="text-[11px] text-rose-700 font-semibold mt-1">
                  CivicMind operates strictly in: <span className="font-extrabold">{SUPPORTED_LOCATIONS_SUMMARY}</span>. Please click a supported city chip above or drag pin into service limits.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center justify-between gap-3 pt-0.5">
            ...
            <button
              ...
              disabled={!serviceValidation.isSupported}
              className={`flex-1 sm:flex-initial sm:min-w-[260px] px-6 sm:px-8 py-2.5 sm:py-3 rounded-xl text-white text-xs sm:text-sm font-black tracking-wide flex items-center justify-center gap-2 transition-all shadow-md ${
                serviceValidation.isSupported
                  ? 'bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 shadow-emerald-700/20 cursor-pointer active:scale-[0.98]'
